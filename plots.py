...
def MID_classification():
    vis.plot_grided(fil, ["M","I","D"],2, vis.points_multi_clas, title="MID statistics for different morphology classifications", new_names = {"bulges":"Bulge", "mergers":"Interacting-Merger"})
    fig = plt.gcf()
    axes = fig.axes
...
def sersic_comparison():
    ...
    for i in range(4):
        axes[i].lines[0].set(markerfacecolor = "#034a8360", markeredgecolor = "#05396f9d", alpha=0.4)
    
    axes[0].set(title="Sersic n", xlabel="n HST", ylabel = "n JWST", xlim= (0,5), ylim=(0,5))
    axes[1].set(title="Sersic radius", xlabel="r HST (arcsec)", ylabel = "r JWST (arcsec)", xlim= (0,1), ylim=(0,1))
    axes[2].set(title="Sersic ellipticity", xlabel="e HST", ylabel = "e JWST", xlim= (0,1), ylim=(0,1))
    axes[3].set(title="Sersic position angle", xlabel=r"$\theta$ HST (rad)", ylabel = r"$\theta$ JWST (rad)", xlim= (0,np.pi), ylim=(0,np.pi))
    fig.suptitle("Sersic fit comparison: HST (F160W) vs JWST ($\lambda_{RFW}=1.37\mu$m)")
    fig.set_size_inches(17.5,5)
    fig.set_layout_engine(layout="tight")
    
def ginim20_class():
    vis.plot_grided(fil, ["S(G, M20)","F(G, M20)"],2, vis.points_multi_clas, title="Standard morphological classification indicators", new_names = {"bulges":"Bulge", "mergers":"Interacting-Merger"})
    fig = plt.gcf()
    axes = fig.axes
